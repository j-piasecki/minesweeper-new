...
public class HubGameLostView extends HubView {
    ...
    public HubGameLostView(final InfoHub parent) {
        super(parent);
        ...
        backButton.setCallback(new Button.ClickCallback() {
            @Override
            public void onClick() {
                HomeView.Transition transition = new HomeView.Transition(RenderView.ViewType.Home);
                transition.setOrigin(new PointF(backButton.getPosition().x + backButton.getSize().x * 0.5f, backButton.getPosition().y + backButton.getSize().y * 0.5f));
    ...
    public void update(PointF position) {
        ...
        backButton.setPosition(new PointF(position.x + RenderView.VIEW_WIDTH * 0.06f, position.y + RenderView.SIZE * 0.22f));
        replayButton.setPosition(new PointF(position.x + RenderView.VIEW_WIDTH * 0.14f + backButton.getSize().x, position.y + RenderView.SIZE * 0.22f));
    ...
    protected void drawContent(Canvas canvas) {
        ...
        canvas.drawText(DataManager.HUB_GAME_LOST, position.x + (RenderView.VIEW_WIDTH - paint.measureText(DataManager.HUB_GAME_LOST)) * 0.5f, position.y + paint.getTextSize() * 1.2f, paint);
